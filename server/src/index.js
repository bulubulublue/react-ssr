...
app.get('*', (req, res) => {
  ...
  matchRoutes(routes, req.path); // 返回将要渲染的组件 [{route:{loadData:[Function],path:'/users',component:[Object]}}]

  const promises = matchRoutes(routes, req.path).map(({ route }) => {
    //如果组件中有loadData方法，则调用
    return route.loadData && route.loadData(store); // 这里将store传递给loadData方法, loadData返回的是一个promise
  });

  // 表明数据加载已经完成，store中已经获取到了请求的数据
